...
function adjustScore(id,week,sch,field,delta,el){if(el&&el.blur)el.blur();const sc=getScore(id,week,sch);sc[field]=Math.max(0,(+sc[field]||0)+delta*STEP_POINTS);updateScoreDisplays(id,week,sch);scheduleSave(250);renderRankings()}
...
/* ===== V52 - BACKUP AUTOMÁTICO DIÁRIO + VERSÃO VISÍVEL ===== */
const LAST_DAILY_KEY="primo_kids_last_daily_backup";
async function dailyBackupIfNeeded(){
  try{
    if(!isAdminAuthenticated()||!cloudReady())return;
    const today=new Date().toISOString().slice(0,10);
    if(localStorage.getItem(LAST_DAILY_KEY)===today){updateConfigLabels();return;}
    const payload={app_id:APP_ID,backup_date:today,data:state,created_at:new Date().toISOString()};
    const url=cloudUrl()+"/rest/v1/primo_daily_backups?on_conflict=app_id,backup_date";
    const r=await authedFetch(url,{method:"POST",headers:{"Content-Type":"application/json","Prefer":"resolution=merge-duplicates,return=minimal"},body:JSON.stringify(payload)});
    if(r.ok){
      localStorage.setItem(LAST_DAILY_KEY,today);
      updateConfigLabels();
      const cutoff=new Date(Date.now()-30*864e5).toISOString().slice(0,10);
      const del=cloudUrl()+"/rest/v1/primo_daily_backups?app_id=eq."+encodeURIComponent(APP_ID)+"&backup_date=lt."+cutoff;
      authedFetch(del,{method:"DELETE",headers:{"Prefer":"return=minimal"}}).catch(()=>{});
    }
  }catch(e){ console.warn("Backup diário falhou (não afeta o app):",e); }
}
function updateConfigLabels(){
  const v=document.getElementById("appVersionLabel"); if(v)v.textContent="Versão "+APP_VERSION;
  const b=document.getElementById("lastDailyBackup"); if(b)b.textContent=localStorage.getItem(LAST_DAILY_KEY)||"ainda não feito hoje";
}
async function initCloud(){
  ...
  try{
    ...
    dailyBackupIfNeeded();          // backup automático 1x por dia (não trava se falhar)
...
async function downloadAllClassifications(){
  ...
  try{
    ...
    const pad=12,titleH=40,subH=22,headerH=420,rowGap=4;
    const gold="#ffd54a",silver="#eef4ff",bronze="#ff9d5c";
    const fixedCard=pad+titleH+subH+6+pad;
    // reserva pro rodapé (AGRADECIMENTO + patrocinadores)
    const spW=W-2*M,spH=Math.round(spW*(300/2000));
    const footerReserve=36+spH+M;                 // agradecimento + faixa + margem
    const availCols=H-M-footerReserve;            // altura útil pras 2 colunas
    // acha o maior rowH que faz as 5 tabelas caberem
    const sorted=[...cats].sort((a,b)=>b.list.length-a.list.length);
    function colsBottomFor(rh){
      const ch=c=>fixedCard+c.list.length*rh;
      let L=M+headerH+gap,R=M;
      sorted.forEach(c=>{const h=ch(c);if(L<=R)L+=h+gap;else R+=h+gap;});
      return Math.max(L,R)-M;
    }
    let rowH=44;for(rowH=44;rowH>=16;rowH--){if(colsBottomFor(rowH)<=availCols)break;}
    // posiciona (mesma masonry) com o rowH escolhido
    const cardH=c=>fixedCard+c.list.length*rowH;
    const leftX=M,rightX=M+colW+gap;let leftY=M,rightY=M;
    const placements=[{kind:"header",x:leftX,y:leftY,w:colW}];leftY+=headerH+gap;
    sorted.forEach(c=>{const h=cardH(c);if(leftY<=rightY){placements.push({kind:"cat",cat:c,x:leftX,y:leftY,w:colW});leftY+=h+gap;}else{placements.push({kind:"cat",cat:c,x:rightX,y:rightY,w:colW});rightY+=h+gap;}});
    ...
    function drawHeader(x,y,w){
      ...
      ctx.fillStyle="#fff";ctx.textBaseline="middle";ctx.fillText(mt,cx,ty+75);ctx.textBaseline="alphabetic";
    }
    function drawCatCard(x,y,w,c){
      const col=CATCOL[c.code]||["#0b4a6e","#5cc4e8"],accent=col[1],h=cardH(c),cx=x+w/2;
      _rr(ctx,x,y,w,h,16);
      let cg=ctx.createLinearGradient(0,y,0,y+h);cg.addColorStop(0,"rgba(10,24,54,.62)");cg.addColorStop(1,"rgba(2,8,23,.68)");ctx.fillStyle=cg;ctx.fill();
      ctx.lineWidth=3;ctx.strokeStyle="rgba(56,189,248,.6)";ctx.save();ctx.shadowColor="rgba(56,189,248,.5)";ctx.shadowBlur=14;ctx.stroke();ctx.restore();
      ctx.textAlign="center";ctx.textBaseline="alphabetic";
      ctx.save();ctx.shadowColor=accent;ctx.shadowBlur=12;ctx.fillStyle="#eaf6ff";
      let cs=28;ctx.font=`italic 900 ${cs}px "Arial Black",Arial,sans-serif`;
      while(ctx.measureText(c.name.toUpperCase()).width>w-2*pad&&cs>18){cs-=2;ctx.font=`italic 900 ${cs}px "Arial Black",Arial,sans-serif`;}
      ctx.fillText(c.name.toUpperCase(),cx,y+pad+cs);ctx.restore();
      if(spacer)ctx.letterSpacing="3px";
      ctx.fillStyle="#7dd3fc";ctx.font='800 15px Arial';ctx.fillText("MAIOR PONTUADOR",cx,y+pad+titleH+2);
      if(spacer)ctx.letterSpacing="0px";
      let ry=y+pad+titleH+subH+6;const rowX=x+pad,rowW=w-2*pad,rh=rowH-rowGap;
      const fs=Math.max(12,Math.min(19,rh*0.52));
      c.list.forEach((s,i)=>{
        ...
        ctx.lineWidth=isTop?2:1;ctx.strokeStyle=isTop?acc:"rgba(90,150,220,.5)";ctx.stroke();
        const cy=ry+rh/2;
        ctx.textBaseline="middle";ctx.textAlign="left";ctx.font=`900 ${fs}px Arial`;ctx.fillStyle=isTop?acc:"#cfe3ff";ctx.fillText(pos+"º",rowX+10,cy);
        const r=rh*0.36,pcx=rowX+10+fs*2.5,ph=photoMap[s.id];
        ...
        ctx.restore();ctx.lineWidth=1.6;ctx.strokeStyle=acc;ctx.beginPath();ctx.arc(pcx,cy,r,0,7);ctx.stroke();
        ctx.textAlign="left";ctx.font=`800 ${fs}px Arial`;ctx.fillStyle="#fff";const nameX=pcx+r+10;
        ctx.fillText(_fit(ctx,String(s.name).toUpperCase(),rowW-(nameX-rowX)-72),nameX,cy);
        ctx.textAlign="right";ctx.font=`900 ${fs}px Arial`;ctx.fillStyle=isTop?acc:"#cfe3ff";ctx.fillText(s.total+" pts",rowX+rowW-10,cy);
    ...
    const spY=H-M-spH,spX=M;
    ctx.textAlign="center";if(spacer)ctx.letterSpacing="10px";ctx.fillStyle="#7dd3fc";ctx.font='900 24px Arial';ctx.fillText("AGRADECIMENTO",W/2,spY-14);if(spacer)ctx.letterSpacing="0px";
